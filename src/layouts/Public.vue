	...
	<div class=" min-h-screen">
		<section class="flex flex-col">
			<header v-if="!loading" class="container flex items-start gap-2  py-4 md:px-0 px-4 bg-transparent">
				<Undo2 v-if="showBackBtn" class="border border-dark p-1 rounded-md cursor-pointer !min-w-[35px] !max-h-[35px] hover:btn_shadow transite " :size="35" @click="useRouter().push(`/${type}/${id}`)" />
...
const showBackBtn = computed(() => useRouter().currentRoute.value.name !== 'type-id')
